...
abstract class DioMixin implements Dio {
  ...
  Future<Response<T>> fetch<T>(RequestOptions requestOptions) async {
    ...
    /// 请求拦截处理 FutureOr: 如果是Future类型就接着往下传，如果是Object类型就终止传递
        ...
        if (state.type == InterceptorResultType.next) {
          ...
          return listenCancelForAsyncTask(
            ...
            Future(() {
              ...
              /// 负责拦截数据处理，和状态类型修改，决定是否继续往下传
              /// 如果有一个没有修改成next状态，那之后的逻辑处理就别想执行了
              interceptor(state.data as RequestOptions, requestHandler);
              return requestHandler.future;
            }),
          );
        } else {
          return state; /// 状态类型不是next，就不做任何处理，直接往下传
        }
      };
    }
    ...
    for (final interceptor in interceptors) {
      final fun = interceptor is QueuedInterceptor
          ? interceptor._handleRequest /// 为什么这里可以调用私有方法？因为：part 'interceptor.dart';
  ...
  static Future<T> listenCancelForAsyncTask<T>(
    ...
    /// 任意一个完成就代表完成了, 有异常则直接抛出异常，结束整个链路
    return Future.any([
